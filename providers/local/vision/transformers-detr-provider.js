...
export class TransformersDetrProvider extends VisualProvider {
  constructor({
    ...
    super('transformers-detr', {
      version: '1.0.0',
      capabilities: [
        { capability: 'object_identity', score: 0.82 },
        { capability: 'scene_context', score: 0.58 },
      ],
      priority: 72,
    ...
    this.loadTimeoutMs = Number(loadTimeoutMs) || 30000;
    this.detector = null;
  ...
  async healthCheck() {
    ...
    if (this._loadFailed) return { status: 'degraded', reason: 'MODEL_LOAD_FAILED' };
    if (this._loadAttempted && !this.detector) return { status: 'loading' };
    return { status: 'ready', modelId: this.modelId };
  }

  async load() {
    if (this.detector) return;
    if (this._loadAttempted && this._loadFailed) {
      // 如果之前加载失败，不重复尝试
      return;
    }
    ...
    if (this.onModelProgress) {
      this.onModelProgress({ type: 'model_load_start', message: '正在准备视觉模型...' });
    }
    ...
    try {
      const loadPromise = (async () => {
        ...
        if (typeof pipeline !== 'function') {
          throw codeError('TRANSFORMERS_PIPELINE_UNAVAILABLE');
        }
        if (lib?.env) {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('MODEL_LOAD_TIMEOUT')), this.loadTimeoutMs);
      });

      await Promise.race([loadPromise, timeoutPromise]);

      if (this.onModelProgress) {
        this.onModelProgress({ type: 'model_load_complete', message: '视觉模型已准备就绪' });
      }
      this._loadFailed = false;
    } catch (error) {
      ...
      if (this.onModelProgress) {
        this.onModelProgress({
          ...
          message: error.code === 'MODEL_LOAD_TIMEOUT' ? '模型下载超时，请检查网络' : '模型加载失败，将使用基础视觉能力',
        });
      }
      console.warn('General Vision model load failed:', error.message);
      // 不抛出错误，允许系统继续使用其他视觉提供者
    }
  }
  ...
  async analyze(image, { capabilities = [] } = {}) {
    if (!capabilities.some(x => x === 'object_identity' || x === 'scene_context')) {
      throw codeError('UNSUPPORTED_VISUAL_CAPABILITY');
    }

    // 如果模型未加载，尝试加载
    if (!this.detector && !this._loadFailed) {
      await this.load();
    }

    // 如果模型加载失败或未加载成功，返回空结果
    if (!this.detector) {
      return createGeneralVisionObservation({
        ...
        confidence: 0,
        limitations: [
          'General Vision model is not available. Using fallback visual capabilities.',
          'Object identity and scene understanding may be limited.',
          'Check network connection or install the model manually.',
        ],
      });
    }
    ...
    try {
      ...
      console.warn('DETR detection failed:', error.message);
      return createGeneralVisionObservation({
        providerId: this.id,
        identity: [],
        scene: [],
        regions: [],
        confidence: 0,
        limitations: ['DETR detection failed. Using fallback visual capabilities.'],
      });
    }
  }

  async #detect(image) {
    const cached = getCache(this, image);
...
export function normalizeDetections(output = [], threshold = 0.5) {
...
export function inferSceneCandidates(detections = []) {
  ...
  for (const [label, need, min] of defs) {
    ...
    if (hits.length >= min) {
      const conf = Math.min(0.86, 0.52 + hits.length * 0.1);
      out.push({ label, confidence: conf, status: conf >= 0.85 ? 'observed' : 'candidate', evidence: { detectedObjects: hits } });
    }
  }
...
function aggregateIdentity(d) {
  ...
  return [...best.values()].sort((a, b) => b.score - a.score).slice(0, 12).map(x => ({
    label: x.label,
    confidence: x.score,
    status: x.score >= 0.85 ? 'observed' : 'candidate',
